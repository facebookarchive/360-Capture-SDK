...
namespace FBCapture { namespace Video {
	...
	bool AMDEncoder::flushEncodedImages()
	{
		AMF_RESULT res = AMF_OK;  // error checking can be added later
	...
	AMF_RESULT AMDEncoder::initialization(const void* texturePtr, const wstring& fullSavePath, int fps)
	{
		...
		if (hr != AMF_OK) {
			DEBUG_ERROR("Failed to initialize AMF Factory");
			return hr;
		}
	...
	bool AMDEncoder::encodeMain(const void* texturePtr, const wstring& fullSavePath, int fps, bool needFlipping)
	{
		AMF_RESULT hr = AMF_OK; // error checking can be added later
		...
		{
			hr = initialization(texturePtr, fullSavePath, fps);
			if (hr != AMF_OK) {
		...
		if (surfaceIn_ == NULL)
		{
			...
			fillSurface(context_, surfaceIn_, needFlipping);
		}
		// encode
	...
	AMF_RESULT AMDEncoder::fillSurface(amf::AMFContext *context, amf::AMFSurface *surface, bool needFlipping)
	{
		AMF_RESULT hr = AMF_OK; // error checking can be added later
		HRESULT hres;
	...
	void AMDEncoder::saveScreenShot(const void* texturePtr, const wstring& fullSavePath, bool needFlipping)
	{
		AMF_RESULT hr = AMF_OK; // error checking can be added later
