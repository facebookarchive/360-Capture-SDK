...
namespace FBCapture { namespace Video {
	...
	void NVEncoder::setEncodeConfigures(const wstring& fullSavePath, uint32_t width, uint32_t height, int fps)
	{
		wstring_convert<std::codecvt_utf8<wchar_t>> stringTypeConversion;
		string videoFileName = stringTypeConversion.to_bytes(fullSavePath);

		// Set encoding configuration
		memset(&encodeConfig_, 0, sizeof(EncodeConfig));
		encodeConfig_.endFrameIdx = INT_MAX;
		encodeConfig_.bitrate = 5000000;
		...
		encodeConfig_.outputFileName = videoFileName.c_str();
		encodeConfig_.fOutput = fopen(encodeConfig_.outputFileName, "wb");
		if (encodeConfig_.fOutput == NULL) {
			DEBUG_ERROR_VAR("Failed to create ", videoFileName.c_str());
			return;
		}
	...
	bool NVEncoder::encodeMain(const void* texturePtr, const wstring& fullSavePath, int fps, bool needFlipping)
	{
		...
		if (!encodingInitiated_ && texturePtr)
		{
			...
			// Set Encode Configs
			setEncodeConfigures(fullSavePath, desc.Width, desc.Height, fps);
	...
	bool NVEncoder::flushEncodedImages()
	{
		...
		if (nvStatus != NV_ENC_SUCCESS) {
			DEBUG_ERROR("Failed to flush inputs from buffer");
			return false;
		}

		nvStatus = deinitialize();
		if (nvStatus != NV_ENC_SUCCESS) {
			DEBUG_ERROR("Failed to release resources");
			return false;
		}
	...
	void NVEncoder::saveScreenShot(const void* texturePtr, const wstring& fullSavePath, bool needFlipping)
	{
		HRESULT hr = E_FAIL;
		...
		if (FAILED(hr)) {
			DEBUG_ERROR("Failed on creating new texture for ScreenShot");
			return;
		}
		...
		if (FAILED(hr)) {
			DEBUG_ERROR("Failed on creating image file");
			return;
		}
